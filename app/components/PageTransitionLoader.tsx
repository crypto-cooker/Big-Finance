...
export const PageTransitionLoader: React.FC<PageTransitionLoaderProps> = ({ children }) => {
        ...
        <div className="fixed inset-0 z-[9999] bg-primary/95 backdrop-blur-sm flex items-center justify-center">
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center">
              <h3 className="text-xl font-semibold text-primary mb-2">{loadingText}</h3>
              <p className="text-secondary text-sm">Please wait while we prepare your experience</p>
            </div>
            
            {/* Progress Bar */}
            <div className="w-64 h-1 bg-panel rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-accent to-accent2 rounded-full animate-pulse" 
                   style={{ 
            ...
            <div className="flex gap-2">
             ...
             {/* Page Content */}
       <div className={isLoading ? 'opacity-50 transition-opacity duration-500' : 'opacity-100 transition-opacity duration-500'}>
         {children}
       </div>
    </>
  );
...
export const LoadingSpinner: React.FC<{
  ...
  if (variant === 'page') {
    return (
      <div className="flex flex-col items-center gap-4">
        <div className="relative">
          <div className={`${sizeClasses[size]} bg-gradient-to-br from-accent to-accent2 rounded-full flex items-center justify-center animate-pulse`}>
            <span className="text-white font-bold text-sm">BF</span>
          </div>
          <div className={`absolute inset-0 ${sizeClasses[size]} border-2 border-accent/30 border-t-accent rounded-full animate-spin`} />
        </div>
        {text && (
          <p className={`${textSizes[size]} text-secondary text-center`}>{text}</p>
        )}
      </div>
    );
  }
  ...
  if (variant === 'button') {
    return (
      <div className="flex items-center gap-2">
        <div className={`${sizeClasses.sm} border-2 border-white/30 border-t-white rounded-full animate-spin`} />
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {text && (
        <p className={`${textSizes[size]} text-secondary`}>{text}</p>
      )}
    </div>
  );
};
